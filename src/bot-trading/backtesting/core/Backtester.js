class Backtester {
  ...
  async runBacktest(data) {
    ...
    for (let i = 50; i < data.length; i++) {
      ...
      if (position) {
        const exitSignal = this.strategy.monitorPosition(
          currentPrice,
          position
        );
        if (exitSignal) {
          const tradeResult = this.closePosition(
            position,
            currentPrice,
            timestamp,
            exitSignal.reason
      ...
      this.equity.push({
        timestamp,
        value: currentEquity / this.config.leverage,
      });
    }
  ...
  calculateMetrics(trades) {
    ...
    const totalProfit = trades.reduce(
      (sum, t) => sum + (t.netPnl > 0 ? t.netPnl : 0),
      0
    );
    const totalLoss = Math.abs(
      trades.reduce((sum, t) => sum + (t.netPnl < 0 ? t.netPnl : 0), 0)
    );
  ...
  calculateSharpeRatio() {
    ...
    for (let i = 1; i < this.equity.length; i++) {
      const return_ =
        (this.equity[i].value - this.equity[i - 1].value) /
        this.equity[i - 1].value;
